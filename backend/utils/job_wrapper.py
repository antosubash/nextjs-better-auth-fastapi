...
async def execute_job_with_logging(  # noqa: PLR0912, PLR0915
    ...
    **kwargs: Any,
    ...
    # Verify job is stored in job store before execution (especially important for long-running tasks)
    ...
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            task = asyncio.create_task(_verify_job_in_store())
            # Store reference to prevent garbage collection
            _ = task
        else:
            await _verify_job_in_store()
    except Exception as e:
        logger.warning(f"Could not verify job {job_id} in store: {e!s}")
    ...
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            task = asyncio.create_task(_mark_job_running())
            # Store reference to prevent garbage collection
            _ = task
        else:
            await _mark_job_running()
    except Exception as e:
        logger.warning(f"Could not mark job {job_id} as RUNNING: {e!s}")
