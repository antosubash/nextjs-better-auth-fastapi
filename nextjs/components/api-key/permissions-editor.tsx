...
export function PermissionsEditor({ value, onChange }: PermissionsEditorProps) {
  ...
  const validatePermissionsStructure = (parsed: unknown): parsed is Record<string, string[]> => {
  ...
  const handleJsonChange = (json: string) => {
    ...
    try {
      if (json.trim()) {
        ...
        if (validatePermissionsStructure(parsed)) {
          onChange(parsed);
        }
      } else {
        onChange({});
  ...
  const renderResourceCard = (resource: string) => {
