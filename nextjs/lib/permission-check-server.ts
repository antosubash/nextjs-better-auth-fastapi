...
export async function requirePermission(
  ...
  if (!result.hasPermission) {
    console.log(`[requirePermission] Access denied: ${result.error || PERMISSION_ERRORS.INSUFFICIENT_PERMISSIONS}`);
    return NextResponse.json(
...
export async function requireAllPermissions(
  ...
  for (const { resource, action } of permissions) {
    ...
    if (!result.hasPermission) {
      console.log(`[requireAllPermissions] Access denied: Missing permission ${resource}.${action} - ${result.error || PERMISSION_ERRORS.INSUFFICIENT_PERMISSIONS}`);
      return NextResponse.json(
        { error: result.error || PERMISSION_ERRORS.INSUFFICIENT_PERMISSIONS },
        { status: 403 }
      );
