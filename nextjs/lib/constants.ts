...
export const JOB_LABELS = {
  ...
  JOB_HISTORY_DESCRIPTION: "View execution history and logs for all jobs",
...
export const JOB_PLACEHOLDERS = {
  JOB_ID: "Enter job ID",
  FUNCTION: "Enter function name (e.g., jobs.example_jobs:function_name)",
  CRON_EXPRESSION: "Enter cron expression (e.g., 0 0 * * *)",
  RUN_DATE: "Select run date",
  START_DATE: "Select start date (optional)",
  END_DATE: "Select end date (optional)",
  WEEKS: "0",
  DAYS: "0",
  HOURS: "0",
  MINUTES: "0",
  SECONDS: "0",
  ARGS: 'Enter arguments as JSON array (e.g., ["arg1", "arg2"])',
  KWARGS: 'Enter keyword arguments as JSON object (e.g., {"key": "value"})',
  TRIGGER_TYPE: "Select trigger type",
} as const;

export const JOB_ERRORS = {
  CREATE_FAILED: "Failed to create job",
  DELETE_FAILED: "Failed to delete job",
  PAUSE_FAILED: "Failed to pause job",
  RESUME_FAILED: "Failed to resume job",
  LOAD_JOBS_FAILED: "Failed to load jobs",
  LOAD_JOB_FAILED: "Failed to load job",
  LOAD_JOB_HISTORY_FAILED: "Failed to load job history",
  JOB_ID_REQUIRED: "Job ID is required",
  FUNCTION_REQUIRED: "Function is required",
  TRIGGER_TYPE_REQUIRED: "Trigger type is required",
  CRON_EXPRESSION_REQUIRED: "Cron expression is required for cron jobs",
  INVALID_CRON_EXPRESSION: "Invalid cron expression",
  INVALID_ARGS: "Invalid arguments format. Must be a valid JSON array.",
  INVALID_KWARGS: "Invalid keyword arguments format. Must be a valid JSON object.",
  INVALID_TRIGGER_TYPE: "Invalid trigger type",
  INTERVAL_REQUIRED:
    "At least one interval field (weeks, days, hours, minutes, seconds) must be greater than 0",
} as const;
...
export const PREDEFINED_JOBS = {
  SELECT_TEMPLATE: "Select a predefined job template",
  USE_TEMPLATE: "Use Template",
  CREATE_CUSTOM: "Create Custom Job",
  TEMPLATES: [
    {
      id: "cleanup_old_data",
      name: "Cleanup Old Data",
      description: "Scheduled job to clean up old data from the database (runs daily at midnight)",
      function: "jobs.example_jobs:cleanup_old_data",
    ...
    },
    {
      id: "health_check",
      name: "Health Check",
      description: "Periodic health check job (runs every 5 minutes)",
      function: "jobs.example_jobs:health_check",
    ...
    },
    {
      ...
      description: "One-time job to send a notification email to a user",
      function: "jobs.example_jobs:send_notification_email",
      trigger_type: "once" as const,
      args: ["user123", "Hello, this is a test notification"],
      kwargs: {},
    },
    {
      id: "process_large_dataset",
      name: "Process Large Dataset",
      description: "One-time job to process a large dataset in batches",
      function: "jobs.example_jobs:process_large_dataset",
      trigger_type: "once" as const,
      args: ["dataset_001"],
    ...
    },
    {
      ...
      description: "Scheduled job to generate reports (runs daily at 2 AM)",
      function: "jobs.example_jobs:generate_report",
      trigger_type: "cron" as const,
      cron_expression: "0 2 * * *",
      args: ["monthly"],
      kwargs: {},
