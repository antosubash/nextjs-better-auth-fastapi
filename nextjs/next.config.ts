...
const defaultCspPolicy = `default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: ${minioOrigin}; font-src 'self' data:; connect-src 'self';`;
const cspPolicy = process.env.NEXT_PUBLIC_CSP_POLICY || defaultCspPolicy;
